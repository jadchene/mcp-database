...
export abstract class BaseSqlAdapter implements SqlDatabaseAdapter {
  ...
  public async listSchemas(maxRows: number): Promise<LimitedMetadataResult<SchemaInfo>> {
    ...
    const result = await this.executeLimitedMetadata("list_schemas", sql, undefined, maxRows);
    return {
  ...
  public async listTables(schema: string | undefined, maxRows: number): Promise<LimitedMetadataResult<TableInfo>> {
    ...
    const result = await this.executeLimitedMetadata("list_tables", query.sql, query.params, maxRows);
    return {
      items: result.rows.map((row) => ({
        schema: String(
          row.schema ??
  ...
  public async describeTable(
    ...
    table: string,
    maxRows: number
  ): Promise<LimitedMetadataResult<ColumnInfo>> {
    if (!table.trim()) {
    ...
    const result = await this.executeLimitedMetadata("describe_table", query.sql, query.params, maxRows);
    return {
      items: result.rows.map((row) => ({
  ...
  public async listIndexes(
    ...
    table: string,
    maxRows: number
  ): Promise<LimitedMetadataResult<IndexInfo>> {
    if (!table.trim()) {
      throw new ApplicationError("INVALID_ARGUMENT", "table must not be empty");
    }
    ...
    const result = await this.executeLimitedMetadata("list_indexes", query.sql, query.params, maxRows);
    return {
      items: result.rows.map((row) => ({
        schema: String(
          row.schema ??
  ...
  private async executeLimitedMetadata(
    ...
    const limitedSql = this.config.type === "oracle"
      ? `${sql}\nFETCH FIRST ${fetchRows} ROWS ONLY`
      : `${sql}\nLIMIT ${fetchRows}`;
    const rows = await this.runWithTimeout(operation, limitedSql, params, () =>
      this.executeRaw(limitedSql, params)
    );
    return {
