...
$defaultConfigPath = Join-Path $resolvedProjectRoot "config\databases.example.json"
$resolvedConfigPath = if ($ExampleConfigPath) { $ExampleConfigPath } else { $defaultConfigPath }
$packageName = "mcp-database-service"
...
try {
    ...
    Invoke-Step -FilePath "npm" -Arguments @("run", "build") -WorkingDirectory $resolvedProjectRoot -Description "Build the MCP database service"
    ...
    try {
        Invoke-Step -FilePath "npm" -Arguments @("uninstall", "-g", $packageName) -WorkingDirectory $resolvedProjectRoot -Description "Remove any previous global installation of mcp-database-service"
    }
    catch {
        Write-Host "Previous global installation was not removed cleanly. Continuing with fresh install."
    }

    Invoke-Step -FilePath "npm" -Arguments @("install", "-g", $tarballPath) -WorkingDirectory $resolvedProjectRoot -Description "Install the packaged mcp-database-service tarball globally"
...
Write-Host "Command: mcp-database-service"
Write-Host "Example start command:"
Write-Host "  mcp-database-service --config $resolvedConfigPath"
Write-Host ""
